...
g.add_argument("--eval_steps", type=int, help="steps between evaluations")
...
def main(args):
    ...
    ADAPTER_MODEL = f"{args.save_dir}/lora_adapter"
    trainer.model.save_pretrained(ADAPTER_MODEL)

    # Load the original model and merge with the adapter
    model = AutoModelForCausalLM.from_pretrained(args.model_id, device_map='auto', torch_dtype=torch.float16)
    model = PeftModel.from_pretrained(model, ADAPTER_MODEL, device_map='auto', torch_dtype=torch.float16)

    # Merge adapter weights with the base model and unload the adapter
